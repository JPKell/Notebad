...
class Log:
        ...
        if log.get('level') <= cfg.log_console_level:
            ...
            for line in log_lines:
                if line != '': 
                    ...
                    print(f"{time}{color} {log.get('name'):8}{close_color}{log.get('module'): <33} {line}")
    ...
    def __perf_console_write(self, log:dict):
        ...
        if cfg.log_performance_to_console:
            ...
            time = dt.datetime.strftime(log.get('timestamp'), '%H:%M:%S.%f')[:-3]
            print(f"{time}{color} {'PERF':8}{close_color}{log.get('module'): <33} {speed_color}{log.get('duration'):<7.5f} sec{close_color} for {log.get('name')}\t")
