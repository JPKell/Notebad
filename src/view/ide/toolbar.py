...
from widgets import NCheckbutton, NButton, NEntry, NLabel, NFrame
...
class Toolbar(NFrame):
    ...
    def _make_find_case_toggle(self):
        ...
        self.case_toggle = NCheckbutton(self, text="Aa", variable=self.case_variable, onvalue=1, offvalue=0,
                                       command=lambda: self.ide.editor.set_find_case(self.case_variable.get()))
    ...
    def _make_find_next_buttons(self):
        self.find_next_btn = NButton(self, text="►", width=1, command=lambda: self.ide.editor.find_text(self.find_entry.get(), direction=1))
        self.find_prev_btn = NButton(self, text="◄", width=1, command=lambda: self.ide.editor.find_text(self.find_entry.get(), direction=-1))
    ...
    def get_current_text(self, *args):
        ...
        self.ide.text.editor.find_text(current_txt)
    ...
    def find_entry_focus(self, *args):
        ...
        if self.ide.text.tag_ranges(SEL):
            self.find_entry.delete(0, 'end')
            self.find_entry.insert(0, self.ide.text.get(SEL_FIRST, SEL_LAST))
        else:
