...
class Toolbar(NFrame):
    ...
    def _make_find_next_buttons(self):
        self.find_next_btn = NButton(self, text="►", width=1, command=lambda: self.view.textbox.editor.find_text(self.find_entry.get(), direction=1))
        self.find_prev_btn = NButton(self, text="◄", width=1, command=lambda: self.view.textbox.editor.find_text(self.find_entry.get(), direction=-1))
